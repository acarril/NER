...
\path [every node/.style={inner sep=0.2cm, outer sep=0.2cm}]
  node (muestra1) at (0, 1.6cm) {Muestra 1: $\{ (y_1,x_1),\ldots,(y_n,x_n) \}$}
  node[align=right] (muestra2) at (0, 0.8cm) {Muestra 2: $\{ (y_1,x_1),\ldots,(y_n,x_n) \}$}
  node (muestradots) at (0,0) {$\vdots$}
  node (muestrak-1) at (0, -0.8cm) {Muestra $k-1$: $\{ (y_1,x_1),\ldots,(y_n,x_n) \}$}
  node (muestrak) at (0, -1.6cm) {Muestra $k$: $\{ (y_1,x_1),\ldots,(y_n,x_n) \}$}
  [xshift=6cm]
  node[draw, minimum width=3cm, minimum height=3cm] (MCO) at (0,0) {MCO}
  [xshift=4cm]
  node (est1) at (0, 1.6cm) {$(\hat\beta_0, \hat\beta_1)_{1}$}
  node (est2) at (0, 0.8cm) {$(\hat\beta_0, \hat\beta_1)_{2}$}
  node (estdots) at (0,0) {$\vdots$}
  node (estk-1) at (0, -1.6cm) {$(\hat\beta_0, \hat\beta_1)_{k-1}$}
  node (estk) at (0, -0.8cm) {$(\hat\beta_0, \hat\beta_1)_{k}$};
...
\begin{scope}[->,>=latex]
	...
	% Salida arriba
	\draw[->] ([yshift=1cm]MCO.east) to [out=10,in=180] (est1.west) ;
	\draw[->] ([yshift=.4cm]MCO.east) to [out=10,in=180] (est2.west) ;
	% Salida abajo
	\draw[->] ([yshift=-1cm]MCO.east) to [out=-10,in=-180] (estk-1.west) ;
	\draw[->] ([yshift=-.4cm]MCO.east) to [out=-10,in=-180] (estk.west) ;
\end{scope}
...
Si bien distintas muestras nos llevaran a estimar distintos parámetros, ...
...
\hlstd{muestras} \hlkwb{<-} \hlnum{500} \hlcom{# número de simulaciones }
\hlstd{beta0} \hlkwb{<-} \hlkwd{numeric}\hlstd{(muestras)} \hlcom{# vector vacío para guardar los interceptos estimados}
\hlstd{beta1} \hlkwb{<-} \hlkwd{numeric}\hlstd{(muestras)} \hlcom{# vector vacío para guardar las pendientes estimadas}

\hlcom{# Parámetros de la población y de la muestra}
\hlstd{a} \hlkwb{<-} \hlnum{.2} \hlcom{# verdadero valor del intercepto}
\hlstd{b} \hlkwb{<-} \hlnum{.5} \hlcom{# verdadero valor de la pendiente}

\hlstd{n} \hlkwb{<-} \hlnum{1000} \hlcom{# tamaño muestral}
\hlstd{x} \hlkwb{<-} \hlkwd{runif}\hlstd{(n,} \hlopt{-}\hlnum{1}\hlstd{,} \hlnum{1}\hlstd{)} \hlcom{# crear una muestra aleatoria de n observaciones para x}

\hlcom{# Comenzar simulaciones (loop)}
\hlkwa{for}\hlstd{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlstd{muestras) \{}
  \hlstd{y} \hlkwb{<-} \hlstd{a} \hlopt{+} \hlstd{b}\hlopt{*}\hlstd{x} \hlopt{+} \hlkwd{rnorm}\hlstd{(n,} \hlnum{0}\hlstd{,} \hlnum{1}\hlstd{)} \hlcom{# Verdadera función generadora de datos pob.}
  \hlstd{estimacion} \hlkwb{<-} \hlkwd{lm}\hlstd{(y} \hlopt{~} \hlstd{x)} \hlcom{# estimar parámetros por MCO}
  \hlstd{beta0[i]} \hlkwb{<-} \hlstd{estimacion}\hlopt{$}\hlstd{coef[}\hlnum{1}\hlstd{]} \hlcom{# guardar el intercepto estimado de esta iteración}
  \hlstd{beta1[i]} \hlkwb{<-} \hlstd{estimacion}\hlopt{$}\hlstd{coef[}\hlnum{2}\hlstd{]} \hlcom{# guardar la pendiente estimada de esta iteración}
\hlstd{\}}

\hlcom{# Graficar densidad de pendientes estimadas}
\hlkwd{ggplot}\hlstd{(}\hlkwd{tibble}\hlstd{(beta1),} \hlkwd{aes}\hlstd{(beta1))} \hlopt{+}
  \hlkwd{geom_vline}\hlstd{(}\hlkwc{xintercept} \hlstd{= b,} \hlkwc{color} \hlstd{=} \hlstr{"gray"}\hlstd{)} \hlopt{+}
  \hlkwd{geom_density}\hlstd{()}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=4in,height=3in]{figs/unnamed-chunk-20-1} 
...
{\centering \includegraphics[width=4in,height=3in]{figs/unnamed-chunk-24-1} 
...
{\centering \includegraphics[width=4in,height=3in]{figs/unnamed-chunk-34-1} 
