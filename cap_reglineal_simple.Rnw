...
Entonces nuestra estimación de $\hat\beta$ es solo una realización de infinidad de posibilidades, como se representa en la \autoref{fig:parametros_distribucion}.
...
Necesitamos que la (o las) variables independientes del modelo tengan algo de variación. Un poco más formalmente,
\begin{equation}
\{x_i, i=1,\ldots,n \} \; \text{no son todos iguales.}
\end{equation}
...
El error $u$ debe tener un valor esperado igual a cero, condicional a los valores de la variable independiente:
\begin{equation}
E(\mu | x) = 0.
\end{equation}
...
¿Cómo llegamos a esta conclusión? Ya habíamos supuesto que la esperanza del error es cero, es decir, que $\E(\mu)=0$.
Mencionamos que no perdemos nada haciendo esto, ya que siempre podemos acomodar el intercepto de manera de asegurarnos que $\E(\mu)=0$. Entonces este no es un supuesto demasiado fuerte.
El punto clave es que necesitamos que $\mu$ y $x$ sean independientes, ya que sólo de esa forma $\E(\mu | x) = E(\mu)$  (que ya supusimos es igual a 0).
Por otro lado, si existe una correlación entre $\mu$ y $x$ esto implicará que $\E(\mu | x) \neq 0$, y habremos violado el supuesto. 
...
El supuesto de media condicional cero es probablemente el supuesto más ``importante'' a la hora de determinar si una regresión lineal entregará parámetros insesgados.
Esto ocurre porque es imposible determinar si el supuesto efectivamente se cumple o no, ya que depende de que se cumplan condiciones que no son directamente comprobables.
Volveremos varias veces sobre este supuesto a lo largo del libro.
...
Finalmente, usando los supuestos RLS.1 a RLS.4 se puede demostrar que los estimadores MCO son insesgados, es decir, que\footnote{La demostración completa corresponde al Teorema 2.1 de \textcite[cap. 2]{wooldridge_introductory_2016}.}
\begin{equation}
\E(\hat\beta_0) = \beta_0 \text{ y } \E(\hat\beta_1) = \beta_1.
\end{equation}

En general, si cualquiera de los RLS.1 a RLS.4 no se cumplen, entonces perderemos la propiedad de insesgamiento. RLS.1 requiere que $y$ y $x$ estén relacionados linealmente. Hay que recordar, sin embargo, que la relación es solamente lineal en \emph{parámetros}, no en variables, por lo que podemos transformar $x$ e $y$ para capturar relaciones no lineales más interesantes.
RLS.2 asume que nuestros datos corresponden a una muestra aleatoria de la población, lo que no siempre se cumple en datos de corte transversal. Por ejemplo, es posible que solamente tengamos datos de micro y pequeñas empresas, o que solamente observemos jefas de hogar de los quintiles más pobres.

RLS.3 es bastante débil, y con mucha seguridad se cumplirá en la gran mayoría de datos. Sin embargo, RSL.4 representa un problema potencial. Si no se cumple que la media condicional del error es 0, esto es equivalente a decir que existe alguna correlación entre $x$ y $\mu$.
...
\section{Varianza muestral de los estimadores}

Además de asegurarnos que la distribución muestral de $\hat\beta$ esté centrada alrededor de $\beta$ ---es decir, que $\hat\beta$ sea insesgado---, es importante saber qué tan dispersa es esta distribución de $\hat\beta$.
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Densidades de estimadores insesgados con distinta varianza}
  \label{fig:estimadores_varianza}
\end{figure}

Para poder calcular la varianza de los estimadores MCO es necesario\footnote{En realidad no es \emph{estrictamente} necesario, pero simplifica mucho los cálculos.} agregar un último supuesto:
...
Usualmente nos interesará $\Var(\hat\beta_1)$, ya que es la varianza del parámetro que acompaña a la variable independiente.
...
\end{question}
\begin{solution}
\url{https://stats.stackexchange.com/questions/153348/other-unbiased-estimators-than-the-blue-ols-solution-for-linear-models}
\end{solution}
\begin{question}
