...
No es necesario creer en mi palabra. A continuación simulamos 500 muestreos aleatorios de 1000 observaciones cada uno. Nos concentraremos únicamente en el parámetro de la pendiente, $\beta_1$. Fijamos el verdadero valor de dicho parámetro en $\beta_1=0.5$ (en el código \verb|b <- 0.5|). Al simular los datos tomados de una población normal, usando los parámetros que hemos definido, estimamos el parámetro $k$ veces y obtenemos $k$ valores para $\hat\beta_1$, los que vamos guardando en el vector \verb|beta1|. Finalmente hacemos un gráfico de densidad con los valores de dicho vector.
