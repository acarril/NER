...
Intuitivamente, la idea es minimizar el error de ajuste entre la recta definida por nuestra elección de $\beta_0$ y $\beta_1$ y los puntos de datos.
...
y_i = \beta_0 + \beta_1 x_i + \mu_i.
\end{equation*}
Dada una muestra de datos (como \verb|simdatos|), llamaremos $\hat\beta_0$ y $\hat\beta_1$ a nuestra estimación de los coeficientes.
Entonces la \kw{línea de regresión por MCO} $\hat y_i$ está definida como
\begin{equation}
\hat y_i = \hat\beta_0 + \hat\beta_1 x_i.
\label{eq:linea_regresion}
\end{equation}
Geométricamente, la predicción $\hat y_i$ del modelo es simplemente la recta que resulta de elegir valores particulares para $\hat\beta_0$ y $\hat\beta_1$, dibujada con azul en la \autoref{fig:MCO}.
...
Podemos escribir la línea de regresión definida en \eqref{eq:linea_regresion} usando los nombres de las variables que nos interesan. Luego estimamos los coeficientes del modelo con \verb|lm()|, esta vez definiendo una fórmula donde \verb|wage| es la variable dependiente y \verb|educ| es la variable independiente.
\begin{align*}
...
\end{equation*}
Por ejemplo, en la estimación anterior hemos obtenido que $\hat\beta_1 = \Sexpr{coefficients(modelo)[1]}$.
Esto puede interpretarse como que un aumento de un año de educación ($x$ aumenta en una unidad) está asociado a un aumento promedio de (casi) \Sexpr{round(coefficients(modelo)[1])} dólares en el salario.
