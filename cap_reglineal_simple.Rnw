...
Hemos visto que el método de MCO encuentra los parámetros del modelo lineal por via de minimizar la suma de errores al cuadrado. Los estimadores MCO ($\hat\beta_0$ y $\hat\beta_1$) que resultan de esta minimización corresponden a las ecuaciones \eqref{eq:MCO_beta0} y \eqref{eq:MCO_beta1}. Con el código anterior calculamos estos estimadores de manera explícita, lo que es útil para entender de manera más profunda qué es lo que estamos haciendo.
